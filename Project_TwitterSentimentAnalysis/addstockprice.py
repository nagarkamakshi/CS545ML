# -*- coding: utf-8 -*-
"""
Created on Sun Nov 24 12:08:27 2019
...
df = pd.read_csv("clusterdata/Google_cluster.csv",index_col=0,encoding='latin-1')
...
def getstockvalue(change, followers, totalfollowers, polarity, confidence, currentprice):
    ...
    print(start)
    apple = web.DataReader("APPL", "google", start)
    type(apple)
    print(apple)
    '''
...
start = datetime.datetime(2019,11,27)
# Let's get Apple stock data; Apple's ticker symbol is AAPL
# First argument is the series we want, second is the source ("yahoo" for Yahoo! Finance), third is the start date, fourth is the end date
apple = web.DataReader('INTC', 'yahoo', start)
#print(apple)
...
df.to_csv('finaldata/Intel_final.csv')
